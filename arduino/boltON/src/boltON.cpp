...
// Pointer to the HardwareSerial instance used for communication with the boltON module
static HardwareSerial* boltONSerial = nullptr;

int32_t sl_bt_api_rx(uint32_t dataLength, uint8_t* data)
{
  if (!data || !boltONSerial) {
    return -1;
  }

  // Receive data from the boltON module
  while (boltONSerial->available()) {
    uint8_t data = (uint8_t)boltONSerial->read();
    sl_bolton_buffer_received_data(1, &data);
  }
...
void sl_bt_api_tx(uint32_t msg_len, uint8_t* msg_data)
{
  ...
  boltONSerial->write(msg_data, msg_len);
}
...
bool sl_bolton_init()
{
  return sl_bolton_init(&Serial);
}

bool sl_bolton_init(HardwareSerial* serial)
{
  if (!serial) {
    return false;
  }
  boltONSerial = serial;
  boltONSerial->begin(115200);
...
void sl_bolton_process()
{
  ...
  // Process events from the received data
  sl_bt_msg_t event;
